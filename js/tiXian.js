...
$(function () {
    ...
    $('#btn-tixian').on('click', function () {
        var keTiXian=$('#tixian-money > div > p > span').html();
        var tiXian=$('#tixian-money > div:nth-of-type(2) p input').val();
        /*提醒框3后自动消失*/
        setTimeout(function () {
            $("#toast").css('opacity','0');
        },3000)
        console.log("可提现："+keTiXian);
        if(tiXian.length==0)
        {
            $("#toast").html("输入金额不能为空,请重新输入");
        }
        else if ((keTiXian-0)<(tiXian-0)){
            $("#toast").html("输入金额不能超过账户余额,请重新输入").fadeIn();
        }
       else if(!$('#tixian-method > div:nth-of-type(2) p input').attr("checked")){
            $("#toast").html("请选择提现方式").fadeIn();
        }
    ...
    $('#tixian-method > div:nth-of-type(2) p input').on('click', function () {
